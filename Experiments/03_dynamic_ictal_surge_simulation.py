...
to construct a synthetic proof-of-concept multi-stage epileptic seizure episode for AI model pre-training?

CRITICAL METHODOLOGICAL TRANSPARENCY & CLINICAL LIMITATIONS (Addressing Credibility Risk 2):
  ...
  - Clinical Validation Status: NOT validated against clinical long-term ambulatory video-EEG/ECG epilepsy datasets.
    Serves exclusively as an exploratory synthetic data generator to evaluate model robust feature learning.
...
# Generate continuous signal using piecewise 1-second chunks driven by hr_curve
ecg_chunks = []
np.random.seed(42)
for chunk_idx in range(DURATION):
    t_start = chunk_idx
    target_hr = float(hr_curve[chunk_idx * FS])
    # Vary HR std slightly across stages (lower HRV during ictal surge)
...
clean_seizure_ecg = np.concatenate(ecg_chunks)
...
# Apply time envelope to EMG tremor (active during tonic-clonic phase 25s-40s)
...
# Plot continuous recording with clinical transparency disclaimer
...
plt.title("Synthetic Proof-of-Concept Seizure Simulation: Autonomic Heart Rate Profile\n"
          "(Rule-Based Phenomenological Approximation - Non-Clinical Proof of Concept)", fontsize=11, fontweight='bold')
...
plt.plot(TIME_AXIS, clean_seizure_ecg, color='cyan', alpha=0.5, linewidth=0.5, label="Underlying Rhythm (No Tremor)")
plt.axvspan(25, 40, color='purple', alpha=0.15)
plt.title("Multi-Stage Synthetic Seizure ECG Waveform (Clean vs Tremor Contaminated)", fontsize=10, fontweight='bold')
